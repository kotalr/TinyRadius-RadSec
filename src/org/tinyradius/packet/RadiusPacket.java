...
public class RadiusPacket {
    ...
    /**
     * Adds a Radius attribute to this packet, using bytes Uses AttributeTypes
     * to lookup the type code and converts the value. Can also be used to add
     * sub-attributes.
     *
     * @param typeName name of the attribute, for example "NAS-Ip-Address"
     * @param value value of the attribute, for example "127.0.0.1"
        ...
        if (attribute.getVendorId() == -1) {
            ...
            for (Iterator i = vsas.iterator(); i.hasNext();) {
                ...
                if (sas.contains(attribute)) {
                    ...
                    if (sas.size() == 1) // removed the last sub-attribute
    ...
    public void removeAttributes(int vendorId, int typeCode) {
        ...
        for (Iterator i = vsas.iterator(); i.hasNext();) {
            ...
            if (sas.size() == 0) // removed the last sub-attribute
            // --> remove the whole Vendor-Specific attribute
            {
                removeAttribute(vsa);
            }
     ...
     * appropriate packet is created. Also sets the type, and the the packet
     * identifier.
     *
     * @param type packet type
     ...
     * @param out output stream to use
     * @param sharedSecret shared secret to be used to encode this packet
     * @param request Radius request packet if this packet to be encoded is a
     ...
     * packet attributes like the RadUser-Password attribute. The method may use
     ...
     *
     * @param sharedSecret shared secret that secures the communication with the
     * other Radius server/client
    ...
    protected byte[] updateReqAuthenticator(String sharedSecret, int packetLength, byte[] attributes) {
        byte[] _authenticator = new byte[16];
        for (int i = 0; i < 16; i++) {
            _authenticator[i] = 0;
        }
        MessageDigest md5 = getMd5Digest();
        ...
        md5.update(_authenticator, 0, _authenticator.length);
     ...
     * @param requestAuthenticator 16 bytes authenticator of the request packet
     * belonging to this response packet
     */
